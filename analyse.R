...
plot(
  as.numeric(x) ~ seq(1950 + 1, 1950 + n),
  type = "l",
  xlab = "Year",
  ylab = "Residuals",
  lwd = 3
...
points((trend_forecast + c(err_pred, f[, 4])) ~ seq(1950 + 1, 1950 + n + l),
       type = "l",
       lty = 3,
       lwd = 3
)
points((trend_forecast + c(err_pred, f[, 5])) ~ seq(1950 + 1, 1950 + n + l),
       type = "l",
       lty = 3,
       lwd = 3
)
...
acf(m$residuals, main = "")

f = forecast(m, lead = l)

trend_forecast = c(l1$fitted.values, cbind(rep(1, (n - d + l)), seq(n - d + l)) %*% matrix(coefficients(l2)))
